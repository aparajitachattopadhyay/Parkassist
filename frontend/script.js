...
(function () {
  ...
  function updateSensor(data) {
    ...
    if (validSensors.length > 0) {
      // Calculate weighted average of x and y based on distance
      let sumW = 0, sumX = 0, sumY = 0;
      validSensors.forEach(s => {
        let rad = s.ang * Math.PI / 180;
        let x = s.val * Math.sin(rad);
        let y = s.val * Math.cos(rad);
        // Weight heavily towards closer objects: 1 / (val^2)
  ...
  function drawTopView(sensor, detections) {
    ...
    function plotSensor(dist, angleDeg) {
        ...
        const oy = baseY - 12 - Math.cos(rad) * Y_dist; // up is negative Y
    ...
    if (detections && detections.length > 0) {
        // Find minimum distance from sensors as fallback depth for shapes
        const fallbackDist = sensor.finalDist != null ? sensor.finalDist : 100; // default 100cm if no sensors

        detections.forEach(d => {
            const objAngle = (d.x - 0.5) * 60; // Approximate 60 deg camera FOV centered on 0, bounded -30 to +30
